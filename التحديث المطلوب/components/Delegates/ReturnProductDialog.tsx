...
type ReturnProductFormValues = z.infer<typeof schema>;
...
interface ReturnProductDialogProps {
  ...
  delegateId: string;
  products: Product[];
  onSubmit: (data: ReturnProductFormValues) => void;
}

export function ReturnProductDialog({ isOpen, onClose, delegateId, products, onSubmit }: ReturnProductDialogProps) {
  ...
  // تحديث مخزون المنتج عند الإرجاع
  ...
  const handleSubmit = (data: ReturnProductFormValues) => {
    try {
      ...
      toast({
        title: 'فشل إرجاع المنتج',
        description: 'حدث خطأ أثناء تسجيل عملية الإرجاع',
        variant: 'destructive',
      });
    }
  };
  ...
  return (
    <Dialog open={isOpen} onOpenChange={onClose}>
      <DialogContent className="sm:max-w-[425px]">
        <DialogHeader>
          <DialogTitle>إرجاع منتج من المندوب</DialogTitle>
        </DialogHeader>
        <Form {...form}>
          <form onSubmit={form.handleSubmit(handleSubmit)} className="space-y-4">
            ...
            <div className="flex items-center justify-end gap-4 pt-6">
              ...
              <Button type="submit">تأكيد الإرجاع</Button>
            </div>
          </form>
        </Form>
      </DialogContent>
